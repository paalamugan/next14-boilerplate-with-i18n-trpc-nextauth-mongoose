...
import redirectConfig from './redirect.config.js';
...
const redirects = async () => {
  return redirectConfig.externals.map(({ source, destination }) => ({
...
const rewrites = async () => {
  const mappedRewrites = redirectConfig.internals.map(({ source, destination }) => ({
    source: source.replace('/:locale', localesMatch),
    destination,
  }));

  const { fallback, beforeFiles, afterFiles: localAfterFiles } = redirectConfig.rewrites;
